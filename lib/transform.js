...
const parse = exports.parse = function(code) {
  return recast.parse(code);
};

const convert = exports.convert = function(ast) {
...
const insert = exports.insert = function(target, nodes, index) {
  target.splice.apply(target, [index || 0, 0].concat(nodes));

  return target;
};

const createImport = exports.createImport = function(varname, modulename) {
  return parse('const ' + varname + ' = require(\'' + modulename + '\');\n').program.body;
};

const findFirstNodeAfter = exports.findFirstNodeAfter = function(ast, code, type) {
  let next = false;
  let result = null;
...
exports.addToArrayInObject = function(ast, objectCode, key, code) {
  ...
  const objectAst = findFirstNodeAfter(ast, objectCode, 'ObjectExpression');
  let ran = false;
...
exports.addLastInFunction = function(ast, search, code) {
  const node = findFirstNodeAfter(ast, search, 'FunctionExpression');
