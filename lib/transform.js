...
exports.addInsideFunction = function(ast, search, afterExpression, code) {
  var node = findFirstNodeAfter(ast, search, 'FunctionExpression');
  ...
  for (var node, i = 0, l = nodes.length; i < l; i++) {
    ...
    if (node.type === 'ExpressionStatement') {
      var nodeCode = recast.print(node).code;
      if(nodeCode.indexOf(afterExpression) > -1) {
        insert(nodes, parse(code).program.body, i + 1);
        break;
      }
    }
  }
