...
var <%= S(authentication[i].name).capitalize().s %>Strategy = require('<%= authentication[i].strategy %>').Strategy;<% if (authentication[i].tokenStrategy) { %>
var <%= S(authentication[i].name).capitalize().s %>TokenStrategy = require('<%= authentication[i].tokenStrategy %>')<% if (authentication[i].tokenStrategyExposedNormally) { %>.Strategy<% } %>;<% }} %>
